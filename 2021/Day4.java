static void main() throws Exception {
    ...
    var boards = Arrays.stream(input, 1, input.length)
                       .map(k -> parseBoard(k, numberRegex))
...
static int[][] parseBoard(String boardLinesText, Pattern numberRegex) {
    return Arrays.stream(boardLinesText.split("\n"))
                 .map(k -> numberRegex.matcher(k).results().map(MatchResult::group).mapToInt(Integer::parseInt).toArray())
                 .toArray(int[][]::new);
}
