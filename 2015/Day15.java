static void main() throws Exception {
    var ingredientRegex = Pattern.compile("capacity (-?\\d), durability (-?\\d), flavor (-?\\d), texture (-?\\d), calories (-?\\d)");
    var ingredientParts = Files.lines(Path.of("input.txt"))
                               .map(ingredientRegex::matcher)
                               .peek(Matcher::find)
...
static int calculateScore(int[][] ingredientParts, int totalAmount, Predicate<List<Integer>> mixturePrecondition) {
    return generateMixtures(ingredientParts.length, totalAmount)
...
static Stream<List<Integer>> generateMixturePart(int currentAmount, int ingredientCount, int totalAmount) {
    return ingredientCount == 1 ? Stream.of(List.of(currentAmount))
                                : generateMixtures(ingredientCount - 1, totalAmount - currentAmount)
                                 .map(list -> {
