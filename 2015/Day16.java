static void main() throws Exception {
    var sues = Files.lines(Path.of("input.txt"))
                    .map(k -> k.split(" ", 3))
    ...
    var part1Result = Arrays.stream(sues)
                            .filter(k -> checkPresents(allPresents, k, _ -> Integer::equals))
                            .mapToInt(Sue::ordinal)
                            .findFirst()
                            .orElseThrow();

    var part2Result = Arrays.stream(sues)
                            .filter(k -> checkPresents(allPresents, k, l -> createPart2ValueTester(l)))
                            .mapToInt(Sue::ordinal)
                            .findFirst()
                            .orElseThrow();
