static void main() throws Exception {
    ...
    var startingItemsRegex = Pattern.compile("Starting items: (.*)");
    var monkeys = Arrays.stream(input.split("\n\n"))
                        .map(k -> parseMonkey(k, startingItemsRegex))
                        .toArray(Monkey[]::new);
...
static Monkey parseMonkey(String data, Pattern startingItemsRegex) {
    var lines = data.split("\n");
    var startingItemsMatcher = startingItemsRegex.matcher(lines[1]);
